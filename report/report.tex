...
The root mean squared (RMS) value for each channel individual as well as the global RMS value was calculated and also included as a feature. RMS represents the average amplitude of a waveform. For example, the UK uses alternating current, with an RMS of 240V, which means it produces the same average power as 240 volts of DC.
...
A classifier capabable of online learning would be very useful, as user feedback could provide a method for crowdsourcing samples for further supervised learning. Crowdsourced data collection for producing data for supervised learning has famously been applied by projects such as reCAPTCHA \cite{captcha}.
